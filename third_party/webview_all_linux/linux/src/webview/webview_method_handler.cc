...
#ifdef GDK_WINDOWING_X11
      // Keep the native webview window above the Flutter view on X11 so the X
      // server routes clicks inside its rect to the webview instead of letting
      // them fall through to the window behind the toplevel.
      GdkWindow *webview_window = gtk_widget_get_window(widget);
