...
#ifdef GDK_WINDOWING_X11
  // On X11, the WebKit widget's GdkWindow is client-side (non-native) by
  // default, so it owns no real X window. Because the plugin routes clicks by
  // punching a hole in the Flutter view's input-shape region
  // (update_flutter_view_input_region), that hole becomes click-through to
  // whatever window sits behind the toplevel on window managers that don't
  // assume a Wayland-style single surface (xfwm4/XFCE, Openbox/LXQt): clicks
  // never reach the webview and the app behind gets raised instead. Promoting
  // the webview's window to a native X window gives it a real X child window
  // that catches pointer events inside that hole. This is a no-op on Wayland,
  // where the original client-side routing already works, so we only do it for
  // X11.
  {
    GdkWindow *webview_window = gtk_widget_get_window(GTK_WIDGET(webview->web_view));
    if (webview_window != nullptr && GDK_IS_X11_WINDOW(webview_window)) {
      gdk_window_ensure_native(webview_window);
      // Keep the native webview window stacked above the Flutter view so the X
      // server delivers pointer events inside its rect to the webview.
      gdk_window_raise(webview_window);
    }
  }
#endif
